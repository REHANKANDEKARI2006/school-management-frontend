    ...
    <RouteGuard allowedRoles={[...ADMIN_GROUP, ROLE.CASHIER, ROLE.ACCOUNTANT]}>
      {loading ? ( // Conditional rendering for skeleton
        ...
        <div className="w-full">
          ...
          <Card className="hidden sm:block shadow-sm border-slate-200">
            <CardHeader>
          ...
          {/* Mobile Layout (Exact theme colors matching desktop) */}
          <Card className="sm:hidden border-slate-200 shadow-sm">
            <CardHeader className="pb-4">
              <div className="flex flex-col gap-3">
                <div className="hidden">
                  <CardTitle className="text-xl font-bold">Fees</CardTitle>
                  <CardDescription>Manage fee categories</CardDescription>
                </div>

                {/* Action Buttons with exact desktop button themes */}
                <div className="flex flex-col gap-2 w-full">
                  <div className="grid grid-cols-2 gap-2 w-full">
                    <Button
                      variant="outline"
                      className="w-full px-1 text-xs font-medium whitespace-nowrap"
                      onClick={() => router.push("/main/fees/structures")}
                    >
                      Manage Structures
                    </Button>
            ...
            <CardContent className="p-4 pt-0">
              {filtered.length === 0 ? (
                ...
                <div className="space-y-3">
                  {filtered.map((fee) => (
                    <div
                      key={fee.fee_category_id}
                      className="bg-white rounded-xl p-3.5 border border-slate-200 shadow-xs flex flex-col gap-2"
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Badge
                            variant={fee.allow_installments ? "active" : "cancelled"}
                          >
                            {fee.allow_installments ? "Allowed" : "Not Allowed"}
                          </Badge>
                      ...
                      {/* Description - Full text wrapping cleanly */}
                      {fee.description && (
                        <p className="text-xs text-muted-foreground pt-1.5 border-t border-slate-100 break-words">
                          {fee.description}
                        </p>
                      )}
                    </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="w-[calc(100vw-32px)] max-w-md sm:max-w-lg p-4 sm:p-6 rounded-2xl bg-white border border-slate-200/90 shadow-xl space-y-0">
          <DialogHeader className="pb-3">
            <DialogTitle className="text-lg font-black text-slate-900 text-center sm:text-left tracking-tight">
