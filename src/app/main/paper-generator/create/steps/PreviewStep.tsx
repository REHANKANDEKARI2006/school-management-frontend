...
export default function PreviewStep({ paper }: Props) {
  ...
  const handleDownload = async () => {
    ...
    try {
      // 1. Get the exact HTML of the preview content!
      const htmlContent = contentRef.current ? contentRef.current.innerHTML : "";
 
      // 2. Dynamically collect all active stylesheet rules currently loaded in the browser document.
      let stylesString = "";
      try {
      ...
      // 3. Compile a completely self-contained, pixel-perfect HTML document wrapper with exact printing dimensions.
      const fullHtml = `
        <!DOCTYPE html>
        <html>
        <head>
      ...
      const url  = await generatePaperPDF(paper.paper_id!, { 
        generate_answer_key: includeKey,
        html: fullHtml
      });
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          ...
          <div className="bg-slate-50 rounded-xl p-4 mb-5 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div>
                <Label className="text-xs font-bold">Include Answer Key</Label>
                <p className="text-[10px] text-slate-400">Appended at end of PDF</p>
              </div>
